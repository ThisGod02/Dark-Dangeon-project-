...
public class Map {
    ...
    private Boolean validate(List<String> lines) {
        ...
        return containsExit(lines) && containsPlayer(lines) && containsEnemies(lines) && lines
    ...
    private Boolean containsEnemies(List<String> lines) {
        for (String line : lines) {
            for (char ch : line.toCharArray()) {
                if (ch == ENEMY_CHARACTER) {
                    return true;
                }
            }
        }
    ...
    public void setMapEnemy(int y, int x){
        map[y][x] = ENEMY_CHARACTER;
    }
